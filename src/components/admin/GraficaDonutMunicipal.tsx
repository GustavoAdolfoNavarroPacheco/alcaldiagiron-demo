...
export interface SegmentoMunicipal {
  id: string
  nombre: string
  valor: number
  color: string
...
interface GraficaDonutMunicipalProps {
  datos: SegmentoMunicipal[]
  titulo?: string
  subtitulo?: string
...
export default function GraficaDonutMunicipal({
  datos,
  titulo = 'Distribución de Inversión por Secretaría',
  subtitulo = 'Participación de las dependencias municipales en la vigencia fiscal',
  etiquetaCentro = 'Presupuesto Total',
  etiquetaUnidades = 'trámites',
  segmentoSeleccionadoId,
  onHoverSegmento,
}: GraficaDonutMunicipalProps) {
  ...
  const arcos: ArcoCalculado[] = useMemo(() => {
    ...
    let anguloInicio = -Math.PI / 2 // 12:00 en reloj
      ...
      <div className="border-b border-slate-100 pb-3">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600">
            {datos.length} Dependencias
          </span>
        </div>
        <p className="mt-0.5 text-xs text-slate-500">{subtitulo}</p>
      </div>

      {/* Área del Diagrama Vectorial SVG */}
            ...
            {arcos.map((arco) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Porcentaje impreso sobre el segmento */}
                  {arco.porcentaje >= 6 && (
                    <text
                      x={arco.centroX}
                      y={arco.centroY}
                  ...
                  >
                    {arco.segmento.nombre.length > 22
                      ? `${arco.segmento.nombre.slice(0, 20)}...`
            ...
            {activo ? (
              <>
                <text x="220" y="165" textAnchor="middle" className="text-[10px] font-semibold fill-slate-400 uppercase tracking-wider">
                  {activo.nombre.length > 16 ? `${activo.nombre.slice(0, 15)}...` : activo.nombre}
                </text>
              ...
              <>
                ...
                <text x="220" y="200" textAnchor="middle" className="text-[9px] font-medium fill-slate-500">
                  {datos.reduce((sum, d) => sum + d.casos, 0)} {etiquetaUnidades}
                </text>
              </>
            )}
          </g>
        ...
        {datos.slice(0, 6).map((d) => (
          ...
          >
            ...
            <span className="text-[10px] text-slate-400 font-medium shrink-0">
              {Math.round((d.valor / (total || 1)) * 100)}%
            </span>
          </button>
        ))}
      </div>
